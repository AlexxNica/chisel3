...
trait RegFactory {
  ...
  def apply[T <: Data](t: T)(implicit sourceInfo: SourceInfo, compileOptions: CompileOptions): T = {
    ...
    pushCommand(DefReg(sourceInfo, reg, Node(reg._parent.get.clock)))
    reg
  }

  /** Creates a register with reset value. Internal API, users should use RegInit instead.
    * TODO: should this be a public API, where there is a specified init of different type than the reg?
    */
  def Init[T <: Data](t: T, init: T)(implicit sourceInfo: SourceInfo, compileOptions: CompileOptions): T = {
    if (compileOptions.declaredTypeMustBeUnbound) {
      Binding.checkUnbound(t, s"t ($t) must be unbound Type. Try using cloneType?")
    }
    val reg = t.chiselCloneType
    ...
    pushCommand(DefRegInit(sourceInfo, reg, Node(reg._parent.get.clock), Node(reg._parent.get.reset), init.ref))
    reg
  }
}

private[core] object Reg extends RegFactory
object RegNext {
  ...
  def apply[T <: Data](next: T, init: T)(implicit sourceInfo: SourceInfo, compileOptions: CompileOptions): T = {
    ...
    val reg = Reg.Init(model, init)  // TODO: this makes NO sense
